...
void ControllersEngine::proceed_controllers_pre(float time_interval)
{
	...
	for ( unsigned int i = 0; i < pre_controlled_objects.size(); ++i )
		if ( pre_controlled_objects[i]->controller != nullptr )
			pre_controlled_objects[i]->controller->control_object( pre_controlled_objects[i] );
}
...
void ControllersEngine::proceed_controllers_post( float time_interval )
{
	...
	for ( unsigned int i = 0; i < post_controlled_objects.size( ); ++i )
		if ( post_controlled_objects[i]->controller != nullptr )
			post_controlled_objects[i]->controller->control_object( post_controlled_objects[i] );
}
